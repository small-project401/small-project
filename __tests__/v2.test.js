...
describe("V2 Routes", () => {
  it("admin can post a new experience item", async () => {
    const responseToken = await mockReq
      .post("/signin")
      .auth("admin", "password");
    const token = responseToken.body.token;
    let obj = {
      touristName: "test_experience_1",
      favFood: "test",
      favLandmark: "test",
      thoughts: "test",
    };
  ...
  it("admin can get a experience item", async () => {
    ...
    const response2 = await mockReq
      .post("/api/v2/experience")
      .send(obj)
      .set("Authorization", `Bearer ${token}`);
    const experienceObject = response2.body;
    const res = await mockReq
      .get(`/api/v2/experience/${experienceObject.id}`)
      .set("Authorization", `Bearer ${token}`);
  ...
  it("admin can get all experience items", async () => {
    const responseToken = await mockReq
      .post("/signin")
      .auth("admin", "password");
    const token = responseToken.body.token;
    ...
    let obj = {
      touristName: "test_experience_3",
      favFood: "test",
      favLandmark: "test",
      thoughts: "test",
    };
    let obj2 = {
      touristName: "test_experience_4",
      favFood: "test",
      favLandmark: "test",
      thoughts: "test",
    };
  ...
  it("admin can update() a experience item", async () => {
    ...
    const response1 = await mockReq
      .post("/api/v1/experience")
      .send(obj)
      .set("Authorization", `Bearer ${token}`);
    const response = await mockReq
      .put(`/api/v1/experience/${response1.body.id}`)
      .send(updatedObj)
      .set("Authorization", `Bearer ${token}`);
  ...
  it("admin can delete() a experience item", async () => {
    ...
    const response1 = await mockReq
      ...
      .set("Authorization", `Bearer ${token}`);
    const response2 = await mockReq
      .delete(`/api/v1/experience/${response1.body.id}`)
      .set("Authorization", `Bearer ${token}`);
